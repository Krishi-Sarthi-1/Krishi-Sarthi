...
const ContactUs = () => {
    ...
    const handleSubmit = (e) => {
        ...
        axios.post('http://localhost:8000/api/v1/auth/contact-us', formObject).then(data => {
            console.log(data);
            setDone(true);
            setName('');
            setEmail('');
        ...
        <div className='h-[460px] py-12 bg-[rgba(255,255,255,0.8)]'>
            <div className='flex justify-center'>
                <div className='flex-1 pl-40'>
                    ...
                    <p className='mt-3 text-gray-500 text-md font-semibold'>lkko, Sector 21, Block C, Lucknow</p>
                    <p className='mt-3 text-gray-500 text-md font-semibold'><i className="text-green-400 fa-solid fa-envelope"></i> Email : <a href="" className='text-green-700 font-semibold'>abhisheksingh1746@gmail.com</a></p>
                </div>
                    ...
                    <form ref={formRef} onSubmit={handleSubmit} className="w-full mt-12 max-w-lg">
                        <div className="flex flex-wrap -mx-3 mb-6">
                            <div className="w-full md:w-2/5 mr-3 border border-green-400 px-3 mb-6 md:mb-0">
                                <input onChange={(e) => setName(e.target.value)} value={name} className="cInput appearance-none block w-full rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="grid-first-name" required type="text" name="name" placeholder="Name*" />
                            </div>
                            <div className="w-full md:w-2/5 ml-3 border border-green-400 px-3">
                                <input onChange={(e) => setEmail(e.target.value)} value={email} className="cInput appearance-none block w-full rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-last-name" type="email" name="email" placeholder="Email" />
                            </div>
                            <div className="w-full md:w-2/5 mt-3 mr-3 px-3 mb-6 border border-green-400 md:mb-0">
                                <input onChange={(e) => setPhone(e.target.value)} value={phone} className="cInput appearance-none block w-full rounded py-3 px-4 mb-3 leading-tight border border-green-100 focus:outline-none focus:bg-white" id="grid-first-name" type="text" name="phone" placeholder="Mobile No.*" />
                            </div>
                            ...
                            <div className="w-full h-20 md:w-[450px] mt-3 mb-6 md:mb-0">
                                {/* <textarea onChange={(e) => setMessage(e.target.value)} value={message} className="form-control block w-full px-0.5 py-0.5 text-base font-normal text-gray-700 bg-white bg-clip-padding rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"
                                    ...
                                    placeholder="Message*"
                                ></textarea> */}
                                <textarea rows={3} cols={10} type="text" name="message" id="phone_number" autocomplete="given-number" class="mt-1 px-2 py-1 focus:ring-indigo-500 border border-green-400 outline-none block w-full shadow-sm sm:text-sm rounded-md" />
                            </div>
                            {done && (
